...
conn=mysql.connector.connect(host='localhost',database='bank',user='root',password='Lakshmipathy@2002')

# Establishing a connection to the MySQL database
cursor = conn.cursor()

print("WELCOME TO PATHI BANK..")
        ...
        def withdraw():
            ...
            for i in range(len(info)):
                if(account_number==info[i][0]):
                    ...
                    cursor.execute("UPDATE accounts SET balance = %s WHERE account_number = %s", (balance, account_number))
                    conn.commit()
